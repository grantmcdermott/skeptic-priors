...
had <- 
  ...
  filter(year <= lubridate::year(Sys.time()) - 1) %>% ## Last year with full annual data as of writing
...
giss_rebase <- mean((giss %>% filter(year >= 1961 & year <= 1990))$giss, na.rm = T)
giss <- 
  giss %>%
...
had_rebase  <- mean((gmst %>% filter(year >= 1871 & year <= 1900))$had, na.rm = T)
gmst <-
  gmst %>%
...
climate <-
  climate %>%
  filter(year >= comm_min_year) %>% ## 1866
  filter(year <= 2100)
