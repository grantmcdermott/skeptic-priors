...
## Create new "trf_less_co2" variable since we will be breaking CO2 out on its own
climate <-
  climate %>%
  mutate(trf_less_co2 = trf - co2)
...
priors_loop <-
  pblapply(1:nrow(priors_df), function(j){
    ...
    source(here("R/Robustness/CO2/jags-loop-co2.R"), local = T) 
...
tcr %>%
  ...
  write_csv("Results/Robustness/tcr-co2.csv")
...
coefs_tab <- 
  ...
  arrange(prior, coef) %>%
  spread(prior, value) %>% 
  mutate(coef = factor(coef, levels = c("beta_co2", "beta", "gamma", "delta", "eta", "alpha", "tcr"))) %>%
  arrange(coef) %>% 
