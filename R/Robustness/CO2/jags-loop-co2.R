...
rcp_loop <-
  ...
  bugs_file <- bugs_model_func_co2 ## CHANGED
  ...
  data_list <- 
    list(
      "N" = N, "had" = clim_df$had, "trf_less_co2" = clim_df$trf_less_co2, 
      "co2" = clim_df$co2, ## NEW
  ...
  inits_list <- 
    function() {
      list(alpha = 0, beta_co2 = 0, beta = 0, gamma = 0, delta = 0, eta = 0, sigma = 0.1, phi = 0.5) ## CHANGED
      }
  
  ## Which parameters should R keep track of (i.e. return the posterior distributions for)?
  parameters <- c("alpha", "beta_co2", "beta", "gamma", "delta", "eta", "sigma", "phi", "y_pred") ## CHANGED
  ...
  if (i == "rcp26") {
    ...
    coefs_tab <-
      ...
      mutate(coef = factor(coef, levels=c("beta_co2", "beta","gamma","delta","eta","alpha","sigma","phi"))) %>% 
    ...
    tcr <- 
      tibble(
        beta_co2 = filter(coefs_df, coef == "beta_co2")$values, ## CHANGED
        prior = paste0(prior_type, convic_type)
        ) 
    tcr$tcr <- tcr$beta_co2 * rf2x ## CHANGED 
    ...
    #   fig_s1 +
    #     ggsave(
    #       file = here("TablesFigures/Untracked/Robustness/PNGs/fig-s1-co2.png"),
    #       width = 8, height = 10
    #       )
    #   fig_s1 +
    #     ggsave(
    #       file = here("TablesFigures/Untracked/Robustness/fig-s1-co2.pdf"),
    #       width = 8, height = 10, 
    #       device = cairo_pdf
    #       )
    #   rm(fig_s1)
...
fig_4_lab <- paste0("fig-4-", prior_type, convic_type, "-co2")
fig_4 +
  ggsave(
