...
abstract: |
  ...
  their beliefs rationally in the face of ongoing climate change. I find 
  ...
  remaining pool of sceptics. I discuss the necessary conditions for consensus
...
Climate change has come to represent a defining policy issue of our age. Yet support for comprehensive climate policy at the global scale remains elusive. Despite decades of accumulated research and an overwhelming scientific consensus, many policy makers and citizens are still openly sceptical about the human role in our changing climate (\cite{oreskes2004beyond}, \cite{anderegg2010expert}, \cite{doran2011examining}, \cite{cook2013quantifying}, \cite{verheggen2014scientists}, \cite{tol2014quantifying}, \cite{cook2016consensus}, \cite{saad2019americans}). What are we to make of this scepticism? And just how much evidence would it take to convince climate sceptics that they are wrong? I attempt to answer these questions within an empirical Bayesian framework. I consider a range of sceptic beliefs and examine how these beliefs (i.e. "priors") modulate a person's responsiveness to observed climate data. 
I find that the data tends to overwhelm all but the most extreme cases. However, I also document the non-linear effect that prior beliefs have on convergence with the mainstream scientific consensus. Even as most priors are overwhelmed by the evidence for climate change, it becomes increasingly difficult to convince the remaining holdouts that they are wrong.
My larger goal is to better understand how sceptics respond to the increasing evidence for human-induced climate change, provided that they update their beliefs rationally. In so doing, I hope to shed light on our current policy impasse and offer some remarks about the possibility for finding common ground in the future.
...
Beliefs about climate change are powerful. They dictate our choices as individuals and policies as societies. Beyond that, our beliefs also affect how we interpret information about the world. For a climate sceptic, as for anyone else, beliefs provide a lens through which new information is subjectively interrogated and made intelligible. Of course, this is not to say that beliefs are immutable. A central theme of this paper is the Bayesian updating process, by which beliefs are altered through exposure to new information. But our responsiveness to this new information will depend greatly on how certain we are about our existing beliefs.
...
The use of a composite forcing series is thus a necessary step to ensure that the model remains physically consistent.^[Volcanic aerosols are an exception because they impart only a transitory level of forcing. This explains why $VOLC$ may be included as a separate component in the regression equation \cite{estrada2013statistically}.] Nonetheless, I show in an alternate specification later in the paper that relaxing this constraint leads to near identical conclusions as the physically correct specification. I also show that the core results do not hinge on the imposition of a common efficacy among different forcing agents.
...
```{r recursive, fig.cap = "Recursive TCR estimates. In each panel, the resulting posterior TCR estimate from a sceptic prior is contrasted with the noninformative case (in black). Solid lines denote means, while shaded regions (or dashed lines) denote 95\\% credible intervals. The recursive estimates are run backwards in time, as if from the perspective of present-day sceptic looking back over an increasing body of evidence. Formally, I start by running regression eq. (\\ref{eq:impliedtcr}) on an approximately 30-year subsample of the data nearest to the present and then iterate backwards. Each pass of the iteration adds another year's worth of data and re-runs the regression to obtain an updated posterior TCR. This recursive process continues until the full historical dataset is encompassed."}
recursive_plot =
  function(tcr_rec) {
