...
sens_ni = rbindlist(lapply(
	grep('tcr', list.files(here('results/sensitivity'), full.names = TRUE), value = TRUE),
	function(x) {
		d = read_fst(x)
		setDT(d)
		if ('series' %in% names(d)) d$run = d$series ## Only really for 'anthro' runs
		d[prior=='ni', 
			.(tcr = paste0(sprintf('%.1f', mean(tcr)), 
										 ' (',
										 q025 = sprintf('%.1f',  quantile(tcr, .025)), 
										 ',',
										 q975 = sprintf('%.1f',  quantile(tcr, .975)),
										 ')'
										 )),
			by = run]
		}
...
```{r}
sens_cols = c('mean', 'q025', 'q975')
sens_ni[, (sens_cols) := lapply(.SD, sprintf)]
```
